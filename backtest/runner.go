...
func (r *Runner) applyATRLeverageAndSize(dec *kernel.Decision, entryPrice float64, ts int64) {
	...
	cfg := kernel.NormalizeLeverageConfig(kernel.LeverageConfig{
	...
	result := kernel.CalcLeverageWithPositionSize(klines, cfg.ATRTimeframe, cfg, equity, stopDistPct)
