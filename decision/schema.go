...
var DataDictionary = map[string]map[string]BilingualFieldDef{
	...
	"TechnicalAnalysis": {
		...
		"divergence.total": {
			NameZH: "背离总数",
			NameEN: "Divergence Total",
			Unit:   "count",
			DescZH: "最新一根K线的背离总数（已应用最小背离数过滤）",
			DescEN: "Total divergences on the latest bar (after minimum-count filtering)",
		},
		"divergence.min_required": {
			NameZH: "最小背离数",
			NameEN: "Divergence Min Required",
			Unit:   "count",
			DescZH: "最低需要的背离数，低于该值则认为未检测到背离",
			DescEN: "Minimum required divergences; if below, treated as no divergence",
		},
		"divergence.passed_min_required": {
			NameZH: "背离过滤通过",
			NameEN: "Divergence Passed Filter",
			Unit:   "",
			DescZH: "是否通过最小背离数过滤（true=保留输出，false=清空）",
			DescEN: "Whether it passed the minimum-count filter (true=kept, false=cleared)",
		},
		"divergence.positive_regular_count": {
			NameZH: "正常背离（正）数量",
			NameEN: "Positive Regular Divergence Count",
			Unit:   "count",
			DescZH: "正常背离（价格更低低点，指标更高低点）数量",
			DescEN: "Count of positive regular divergences (price lower low, indicator higher low)",
		},
		"divergence.negative_regular_count": {
			NameZH: "正常背离（负）数量",
			NameEN: "Negative Regular Divergence Count",
			Unit:   "count",
			DescZH: "正常背离（价格更高高点，指标更低高点）数量",
			DescEN: "Count of negative regular divergences (price higher high, indicator lower high)",
		},
		"divergence.positive_hidden_count": {
			NameZH: "隐藏背离（正）数量",
			NameEN: "Positive Hidden Divergence Count",
			Unit:   "count",
			DescZH: "隐藏背离（价格更高低点，指标更低低点）数量",
			DescEN: "Count of positive hidden divergences (price higher low, indicator lower low)",
		},
		"divergence.negative_hidden_count": {
			NameZH: "隐藏背离（负）数量",
			NameEN: "Negative Hidden Divergence Count",
			Unit:   "count",
			DescZH: "隐藏背离（价格更低高点，指标更高高点）数量",
			DescEN: "Count of negative hidden divergences (price lower high, indicator higher high)",
		},
		"divergence.indicators": {
			NameZH: "背离指标映射",
			NameEN: "Divergence Indicator Map",
			Unit:   "",
			DescZH: "每个指标的背离结果，值为 bars_ago（0=无，>0=该背离距离当前的K线数）",
			DescEN: "Per-indicator divergence map; value is bars_ago (0=none, >0=bars back from current bar)",
		},
		"divergence.events": {
			NameZH: "背离事件列表",
			NameEN: "Divergence Events",
			Unit:   "",
			DescZH: "背离事件列表，包含 indicator/type/bars_ago",
			DescEN: "Divergence event list with indicator/type/bars_ago",
		},
		"pattern.detected": {
			NameZH: "检测到的形态",
