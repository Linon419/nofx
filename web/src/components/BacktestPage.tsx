...
export function BacktestPage() {
  ...
  const strategyHasDynamicCoins = useMemo(() => {
    ...
    if (
      ...
      (coinSource.use_ai500 || coinSource.use_oi_top || coinSource.use_otc_top)
    ) {
      return true
    }
    ...
    if (!srcType) {
      if (coinSource.use_ai500 || coinSource.use_oi_top || coinSource.use_otc_top) {
        return true
      }
    }
  ...
  const coinSourceDescription = useMemo(() => {
    ...
    if (!sourceType) {
      const mixed =
        (cs.use_ai500 && cs.use_oi_top) ||
        (cs.use_ai500 && cs.use_otc_top) ||
